...
Application One is a containerized Spring Boot application designed to run on Kubernetes clusters across Dev, UAT, and Production environments. The repository includes automated workflows for continuous integration, container image building, and deployment orchestration.
...
```bash
# Build and run
java -jar target/application-one-*.jar

# Run with specific profile
java -Dspring.profiles.active=dev -jar target/application-one-*.jar
```
...
```bash
# Build image locally
docker build -t application-one:latest .

# Run container
docker run -p 8080:8080 application-one:latest
```
...
├── helm/
│   └── application-one/        ← Kubernetes Helm chart
│       ├── Chart.yaml
│       ├── values.yaml
│       ├── values-dev.yaml
│       ├── values-uat.yaml
...
- Compiled JAR file in `target/application-one-*.jar` (not pushed to registry)
...
5. **Build and Push Image** — Docker build & push with tags:
   - `ghcr.io/saghosh8/application-one:latest`
   - `ghcr.io/saghosh8/application-one:<commit-sha>`
   - `ghcr.io/saghosh8/application-one:<input-release-tag>`
6. **Deploy with Helm** — Apply chart to Dev cluster:
   ```bash
   helm upgrade --install application-one helm/application-one \
     --values helm/application-one/values-dev.yaml \
...
- Container image in GHCR (`ghcr.io/saghosh8/application-one:*`)
...
Output: `target/application-one-*.jar`
...
**Chart Location:** `helm/application-one/`
...
- **Ingress** — External access via `application-one.example.com`
...
# UAT
helm upgrade --install application-one helm/application-one \
  --values helm/application-one/values-uat.yaml \
  --set image.tag=1.0.0-release-abc1234 \
  -n uat --create-namespace

# Prod
helm upgrade --install application-one helm/application-one \
  --values helm/application-one/values-prod.yaml \
  --set image.tag=1.0.0-release-abc1234 \
...
# View logs
kubectl logs -n dev -l app=application-one -f

# Test endpoint
curl http://application-one.dev.example.com/actuator/health
```
...
```yaml
image:
  repository: ghcr.io/saghosh8/application-one
  tag: "latest"
...
ingress:
  enabled: true
  host: application-one.example.com
...
- Check JAR file exists: `target/application-one-*.jar`
...
- Validate Helm values: `helm template application-one helm/application-one/`
